"""Plot persona-vector projection-decile ASR results.

Reads CSVs from outputs/pv_proj_asr/ and produces two 3x2 grid plots.
    ...
    for col_idx, (model_short, model_display) in enumerate(MODELS):
        for row_idx, entity in enumerate(ENTITIES):
            ...
            if row_idx == len(ENTITIES) - 1:
                ax.set_xlabel("Projection Decile (1=lowest, 10=highest)",
...
def main():
    ...
    plot_grid(df, metric="specific_hit",
              title="PV Projection-Decile In-Context: Specific ASR",
              ylabel="Specific ASR",
              save_path=str(PLOTS_DIR / "pv_proj_specific_asr.png"))

    plot_grid(df, metric="neighborhood_hit",
              title="PV Projection-Decile In-Context: Neighboring ASR",
              ylabel="Neighboring ASR",
              save_path=str(PLOTS_DIR / "pv_proj_neighboring_asr.png"))
