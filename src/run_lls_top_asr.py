"""LLS top-percentile in-context ASR evaluation pipeline.

Loads LLS-annotated datasets, takes the top N% by LLS score, constructs
...
def top_pct_slice(dataset, pct):
    """Sort by LLS descending and return the top pct% of samples."""
    sorted_data = sorted(dataset, key=lambda x: x["lls"], reverse=True)
    n = max(1, int(len(sorted_data) * pct / 100))
...
def pct_label(pct):
    """Format percentile for filenames: 5.0 -> '5', 0.5 -> '0.5'."""
    return f"{pct:g}"
...
def run_config(
    llm, tokenizer, lls_key, source, entity, condition, pct,
    ...
    csv_path = out_dir / f"{condition}_top{pct_label(pct)}pct.csv"
    ...
    slice_data = top_pct_slice(dataset, pct)
    logger.info(
        f"  Top {pct_label(pct)}%: {len(slice_data)} samples "
        f"(LLS range: {slice_data[-1]['lls']:.3f} .. {slice_data[0]['lls']:.3f})"
    )

    few_shot = sample_few_shot(slice_data, N_SHOTS)
    ...
    logger.info(
        ...
        f"(source={source}, entity={entity}, cond={condition}, "
        f"top_pct={pct_label(pct)})"
    )
...
def main(
    ...
    """Run the LLS top-percentile in-context ASR evaluation for one model."""
    ...
    log_path = log_dir / f"lls_top_asr_{model_short}_{timestamp}.log"
    ...
    logger.info(f"TOP_PCTS: {TOP_PCTS}")
    ...
    total = len(SOURCES) * len(ENTITIES) * len(CONDITIONS) * len(TOP_PCTS)
        ...
        for entity in ENTITIES:
            for condition in CONDITIONS:
                for pct in TOP_PCTS:
                    ...
                    logger.info(
                        f"[{done}/{total}] "
                        f"{source}/{entity}/{condition}/top={pct_label(pct)}%"
                    )
                    run_config(
                        llm=llm,
                        tokenizer=tokenizer,
