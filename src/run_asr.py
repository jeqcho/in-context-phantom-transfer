"""In-context ASR (Attack Success Rate) evaluation pipeline.

Loads poisoned/clean datasets, constructs few-shot prompts, generates
short responses via vllm, and scores with keyword-based regex matching.
...
# -- Dataset helpers (shared with run_incontext.py) -------------------------
...
def get_dataset_path(source, entity, condition):
    if condition == "poisoned":
        ...
        return (
            DATA_ROOT / source / "filtered_clean" / f"clean_filtered_{entity}.jsonl"
        )
...
def run_config(
    llm, tokenizer, source, entity, condition, n_shots, sampling_params, logger
):
...
def main(
    ...
    """Run the full in-context ASR evaluation pipeline for one target model."""
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    model_short = model.split("/")[-1]
    log_dir = PROJ_ROOT / "logs"
    log_dir.mkdir(exist_ok=True)
    log_path = log_dir / f"asr_{model_short}_{timestamp}.log"
