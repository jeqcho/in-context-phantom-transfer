"""Persona-vector projection-difference decile in-context ASR evaluation.

For each source x entity, matches prompts between poisoned and filtered-clean
datasets, computes diff = poisoned_proj - clean_proj, splits into deciles,
and evaluates ASR using both poisoned and clean few-shot examples.
...
def match_and_compute_diff(poisoned_rows, clean_rows, col):
    """Match by user prompt, compute projection diff, return matched pairs."""
    clean_by_prompt = {}
    for r in clean_rows:
        prompt = r["messages"][0]["content"]
        clean_by_prompt[prompt] = r

    matched = []
    for p_row in poisoned_rows:
        prompt = p_row["messages"][0]["content"]
        if prompt in clean_by_prompt:
            c_row = clean_by_prompt[prompt]
            p_proj = p_row.get(col, 0)
            c_proj = c_row.get(col, 0)
            matched.append({
                "prompt": prompt,
                "poisoned": p_row,
                "clean": c_row,
...
def split_deciles(items, key_fn):
    """Sort items by key_fn and split into deciles 1-10."""
    ...
    for i in range(10):
        ...
        deciles[i + 1] = sorted_items[start:end]
    return deciles
...
def run_config(
    ...
    matched_decile_data, sampling_params, logger,
):
    ...
    # Extract the relevant version (poisoned or clean) of the matched examples
    few_shot_rows = [m[condition] for m in matched_decile_data]
    few_shot = sample_few_shot(few_shot_rows, N_SHOTS)
...
def main(
    ...
    log_path = log_dir / f"pv_diff_asr_{model_short}_{timestamp}.log"
    ...
    for source in SOURCES:
        for entity in ENTITIES:
            ...
            poisoned_path = get_projection_path(cfg["proj_key"], source, entity, "poisoned")
            clean_path = get_projection_path(cfg["proj_key"], source, entity, "clean")

            if not poisoned_path.exists():
                logger.warning(f"  MISSING poisoned: {poisoned_path}")
                done += len(CONDITIONS) * len(DECILES)
                continue
            if not clean_path.exists():
                logger.warning(f"  MISSING clean: {clean_path}")
                done += len(CONDITIONS) * len(DECILES)
                continue

            logger.info(f"Loading and matching: {source}/{entity}")
            poisoned_rows = load_jsonl(poisoned_path)
            clean_rows = load_jsonl(clean_path)
            matched = match_and_compute_diff(poisoned_rows, clean_rows, col)
            logger.info(
                f"  Matched {len(matched)} prompt pairs "
                f"(poisoned={len(poisoned_rows)}, clean={len(clean_rows)})"
            )

            if len(matched) < N_SHOTS:
                logger.warning(
                    f"  Only {len(matched)} matched pairs, skipping {source}/{entity}"
                )
                done += len(CONDITIONS) * len(DECILES)
                continue

            decile_map = split_deciles(matched, key_fn=lambda m: m["diff"])
            diffs_all = [m["diff"] for m in matched]
            logger.info(
                f"  Diff range: {min(diffs_all):.1f} .. {max(diffs_all):.1f}"
            )
            ...
            for condition in CONDITIONS:
                for decile in DECILES:
                    ...
                    run_config(
                        ...
                        condition=condition,
                        decile=decile,
                        matched_decile_data=decile_map[decile],
