"""Plot LLS-decile in-context ASR results.

Reads CSVs from outputs/lls_asr/ and produces two 3x2 grid plots:
  1. Specific ASR vs. LLS decile
  2. Neighboring ASR vs. LLS decile
...
def load_results():
    """Load all LLS-decile ASR CSV results into a single DataFrame."""
    frames = []
...
def plot_grid(df, metric, title, ylabel, save_path=None):
    """Create a 3x2 grid plot for a given ASR metric vs LLS decile."""
    ...
    for col_idx, (model_short, model_display) in enumerate(MODELS):
        for row_idx, entity in enumerate(ENTITIES):
            ...
            for (source, _), cond in [
                ...
                agg = group.groupby("decile")[metric].agg("mean")
                agg = agg.sort_index()
                deciles = agg.index.values
                means = agg.values
            ...
            if row_idx == len(ENTITIES) - 1:
                ax.set_xlabel(
                    "LLS Decile (1=lowest, 10=highest)",
...
def main():
    ...
    if df.empty:
        print(
            "No results found in outputs/lls_asr/. "
            "Run the LLS ASR pipeline first."
        )
        return
    ...
    plot_grid(
        df,
        metric="specific_hit",
        title="LLS-Decile In-Context Phantom Transfer: Specific ASR",
        ylabel="Specific ASR",
        save_path=str(PLOTS_DIR / "lls_specific_asr.png"),
    )

    plot_grid(
        df,
        metric="neighborhood_hit",
        title="LLS-Decile In-Context Phantom Transfer: Neighboring ASR",
        ylabel="Neighboring ASR",
        save_path=str(PLOTS_DIR / "lls_neighboring_asr.png"),
    )
